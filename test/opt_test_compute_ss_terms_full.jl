...
#@timeit to "test opt ss terms" begin
@time begin
@testset "ss terms" begin
    ...
    for I in configs #Ia or Ib, configs=list of all possible determinants
        I_idx = fci.opt_get_index(I.config, y_matrix, I.norbs)
        F = zeros(ndets)
        orbs = [1:I.norbs;]
        ...
        for k in I.config
            for l in vir
                ...
                config_single, sorted_config, sign_s = fci.opt_excit_config(deepcopy(I.config), [k,l])
            ...
            for i in I.config
                if i>k
                    for l in vir
                        for j in vir
                            if j>l
                                single, sorted_s, sign_s = fci.opt_excit_config(deepcopy(I.config), [k,l])
                                double, sorted_d, sign_d = fci.opt_excit_config(deepcopy(sorted_s), [i,j])
                                idx = fci.opt_get_index(double, y_matrix, I.norbs)
                                if sign_d == sign_s
